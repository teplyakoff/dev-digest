...
- `.mcp.json` — **комітиться** (рішення користувача), плюс `mcp/README.md` документує
  ...
  агентської сесії в цьому репо, включно з `implementation-planner`,
  `researcher` і рев'ю-агентами, яким ці інструменти не потрібні. Це і є
  причина, чому токен-бюджет нижче — не косметика.

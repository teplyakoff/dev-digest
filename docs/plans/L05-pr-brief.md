...
— обидва `Spec ID: SPEC-07`, `Status: draft`, стан на комміті `c0eff32`.
Повний перелік критеріїв — **об'єднання двох файлів**: AC-1…AC-35, AC-58…AC-67,
NFR-1…NFR-5, NFR-8 (сервер) + AC-36…AC-57, NFR-6, NFR-7 (клієнт) = 67 AC, 8 NFR.
...
1. Текст пов'язаного issue береться живим запитом через
   ...
   default спеки).
2. Поріг 8 000 токенів авторизується виміром на кроці S6, а не приймається на
...
collect (код) → budget (код) → один структурований виклик → ground (код) →
persist. Чужі дані модуль бере **тільки через композиційний корінь**:
`container.blast` (нове), `container.intent` (наявне, плюс новий метод для тексту
issue), `container.projectContext`, `container.reviewRepo`, `container.tokenizer`.
Сховище — наявна порожня таблиця `pr_brief`, розширена ALTER-ом до колонкової
форми `pr_intent`. Клієнт додає презентаційну `PrBriefCard` над `briefGrid` на
Overview і новий параметр URL `?file=<path>`, який відкриває вкладку зі змінами з
розгорнутою карткою потрібного файлу.
...
- **`*.it.test.ts` = DB-backed** (testcontainers, потрібен Docker); решта
  герметична. Розбиття CI-набору ключується на імені файлу.
- **Тенантність.** `pr_brief`, як і `pr_intent`, **не має `workspace_id`**
  ...
  входу в сервіс; `prId` із запиту, що дійшов до репозиторію без цього, — це
...
- **2026-08-06** — поріг, армований лише на фікстурах, «на трьох реальних PR
  ...
  сама обирає собі вхід. → S6 існує саме тому.
...
- **2026-08-08** — `getPrFiles` не має `ORDER BY`: «порядок файлів PR» не існує,
  сортувати явно в точці використання. → S5 сортує пофайлову статистику за
  `additions+deletions` DESC перед капом у 50.
...
- **2026-08-05** — тест, що передає проп руками, нічого не доводить про те, чи
  його хтось передає (значок скілів «зеленів» цілий лесон). → AC-36, AC-52,
  AC-54, AC-55 перевіряються на `OverviewTab` із **мокнутих даних API**.
...
| S6 | `server/src/tools/measure-brief-input.ts` | `.claude/skills/onion-architecture/SKILL.md` §9 | CLI — це теж транспорт: він викликає сервіс і ніколи не тягнеться повз нього в репозиторій |
...
| S8 | `server/src/modules/brief/pipeline/prompt.ts`, `pipeline/schema.ts` | `.claude/skills/security/SKILL.md` — *Agentic AI Security (OWASP 2026)*; `.claude/skills/onion-architecture/SKILL.md` §11 | ASI01: кожен блок зовнішнього тексту йде в промпт обгорнутим і обмеженим; гард **імпортується** з рушія — копія тексту ламає інваріант «правило одне» |
| S9, S10 | `server/src/modules/brief/service.ts` | `.claude/skills/onion-architecture/SKILL.md` §7; `.claude/skills/security/SKILL.md` — *A09 — Logging and Alerting* | сервіс не знає про HTTP і не пише SQL; лог фіксує **композицію** — вид джерела, ref і розмір, — і жодного байта тіла PR, issue чи документа |
| S11 | `server/src/modules/brief/routes.ts`, `server/src/modules/index.ts` | `.claude/skills/fastify-best-practices/SKILL.md` — *Core Principles*; `.claude/skills/onion-architecture/SKILL.md` §9 | схема оголошується на маршруті (`params` і `response`), ніколи `Schema.parse` у тілі хендлера; хендлер робить рівно три речі — розібрати, делегувати, змапити код статусу |
| S12 | `server/test/brief-*.test.ts`, `server/test/brief.it.test.ts` | `.claude/skills/onion-architecture/SKILL.md` §12 | тест кільця 2 бере override-двійники й **не** бере базу; DB-залежні файли названі окремо, щоб швидка смуга виключала їх за іменем; асертити записаний вихід, а не кількість викликів |
...
- Що додається (імена фіксовані, далі на них посилається весь план):
  ...
  - `PrBriefRecord = z.object({ pr_id, what, why, risk_level, risks: z.array(Risk),
    ...
    dropped_blocks: z.array(z.string()), head_sha, provider, model, derived_at,
...
- Risk: `pnpm lint` тримає onion-кільця (шість винятків забейслайнено) — новий
  імпорт із сусіднього модуля впаде саме тут, і це бейслайн, що працює як задумано, а не привід
  його розширювати.

**S4 — збирач входу, частина 1: детерміновані джерела**
...
- Збирає шість джерел (AC-1) у типізований `CollectedInput` із **іменованими
  блоками** (`intent`, `blast-symbols`, `blast-endpoints`, `blast-crons`,
  `diff-stats`, `file-stats`, `context-docs`, `linked-issue`) — імена блоків потім
  є одиницею відкидання (S7) і рядками у `dropped_blocks` (AC-25):
  1. рядок intent — через `container.intent` (S9 вирішує, деривувати чи ні);
...
- Недоступні входи накопичуються в `unavailable: string[]`, що доживає до
  відповіді (AC-59).
...
**S5 — збирач, частина 2: детермінований рендер блоків**
...
- Кожен блок рендериться у текст **до** виміру, бо NFR-1 міряє саме зібрані
  повідомлення. Тут же живуть капи, які спека називає рівнями відкидання, у їхній
  «повній» формі: `MAX_CALLERS_PER_SYMBOL_IN_BRIEF = 5` застосовується
  **на символ** (`capPerSymbol`, не `slice(0,5)` по плоскому списку —
  `server/INSIGHTS.md`, 2026-08-13), `MAX_FILE_STATS = 50` — після сортування S4.
- Done when: рендер детермінований (двічі поспіль — той самий текст), кожен блок
  має стабільне ім'я.
- Verify: `cd server && pnpm exec vitest run test/brief-sources.test.ts`
- Risk: недетермінований порядок (Map/Set-ітерація) робить кеш і тести
  мерехтливими; сортувати явно всюди, де порядок видно.

**S6 — ВИМІР: авторизувати 8 000 реальними даними**
...
- Skript бере `--pr <uuid>` (можна кілька), збирає вхід через сервіс і друкує
  **поблочно**: ім'я блоку, символи, токени `container.tokenizer.count()`, і
  підсумок. Жодного модельного виклику; `--dry` за замовчуванням і іншого режиму
  немає.
...
- **Умова зупинки, а не усна домовленість.** Нехай `M` — медіана трьох підсумків:
  - `4 000 ≤ M ≤ 16 000` → константа лишається `BRIEF_TOKEN_BUDGET = 8_000`, а
    три виміряні числа записуються коментарем над нею («виміряно на PR …, дата»).
  - `M < 4 000` або `M > 16 000` → **крок зупиняється**. Число не правиться в
    коді: NFR-1 належить спеці, і зміна порогу — це зміна вимоги. Повернути
    вимір `spec-creator` і не починати S7.
...
- Done when: скрипт відпрацював на трьох PR і надрукував поблочні числа, або
  задокументовано, чому не зміг.
...
**S7 — токен-бюджет і рівні відкидання**
- Файли: `server/src/modules/brief/pipeline/budget.ts` (нове) ·
  `server/src/modules/brief/constants.ts` (доповнено)
- Skills: `.claude/skills/onion-architecture/SKILL.md` §7
- Чиста функція `fitToBudget(blocks, tokenizer, budget): { blocks, dropped: string[] }`
  — жодного контейнера, жодного I/O, тому тестується без бази.
...
- Рівні застосовуються по одному й **тільки доки поточний не вичерпано** (AC-23):
  ...
  4. викликачі понад п'ять **на символ** (AC-64);
...
- Кожне відкидання додає ім'я блоку в `dropped` (AC-25, NFR-8); після всіх рівнів
  вхід усе ще понад бюджет → кинути доменну помилку **до** будь-якого виклику
  моделі (AC-26).
- Done when: `test_brief_budget`, `test_brief_budget_order`,
  `test_brief_budget_levels` зелені; у тесті AC-26 стаб `llm()` **кидає** при
  виклику — доводиться відсутність виклику, а не його дешевизна (прецедент
  `06-project-context.md`, NFR-4).
- Verify: `cd server && pnpm exec vitest run test/brief-budget.test.ts`
- Risk: «рівень 3 спрацював, а рівень 2 мав ще матеріал» — це і є тест AC-23;
  реалізація, що застосовує рівні одним прогоном по списку, його провалить.

**S8 — промпт і модель-фейсінг схема**
...
- Кожен блок недовіреного входу йде через `wrapUntrusted(label, text)` (AC-35,
  NFR-4). **Дві нові недовірені поверхні**, яких раніше в промптах не було: шляхи
  з індексу репозиторію та маршрути endpoints — обидві всередині обгорток.
- `BriefExtraction` (schema.ts) — модель-фейсінг і **навмисно вужча** за контракт:
  ...
  Ані `dropped_blocks`, ані `attempts`, ані провенансу — сервер знає це сам, і
  схема не дає моделі місця, куди їх вигадати (прийом `intent/pipeline/schema.ts:9-19`).
  `review_focus[]` — `{path, reason}`, поля рядка не існує (AC-13).
- **Без `.max()` у самій схемі** — стеля застосовується після парсу зрізом
  (`intent/pipeline/schema.ts:21-31` пояснює, у що обійшлася стеля в схемі).
- Done when: `test_brief_prompt_guard` зелений: кожен блок обгорнутий, гард —
  останній у системному повідомленні, нуль незагорнутих шляхів.
...
- Done when: `test_brief_call`, `test_brief_cost` зелені: рівно один виклик,
  форма відповіді, другий раунд є / третього немає, `attempts ≤ 2`, невідома
  вартість не сплющена в нуль.
...
- Skills: `.claude/skills/onion-architecture/SKILL.md` §7 ·
  `.claude/skills/security/SKILL.md` — *A09 — Logging and Alerting*
- Allowlist будується з `changed_files`, шляхів символів, шляхів викликачів і
  маршрутів endpoints відповіді blast (AC-7); на `status === 'degraded'` —
  зі шляхів `pr_files` (AC-8).
- Ризик або елемент focus, що посилається поза allowlist, **відкидається**
  (AC-9, AC-10) — це дзеркало `groundFindings` (`reviewer-core/src/grounding.ts:52-83`):
  перевіряється належність до списку, а не збіг тексту.
- Рядок логу з часткою `N/M` пишеться **безумовно** — і коли відкинуто, і коли ні
  (AC-11, NFR-5; правило з `server/INSIGHTS.md`, 2026-08-06).
- Якщо відкинуто **всі** ризики: `risk_level` від моделі зберігається (AC-12), а
  `risks_grounded: false` їде у відповіді й у рядку БД (AC-58).
- **Заземлення — не захист від інжекції.** Зловмисно названий файл, що є в
  allowlist, цю перевірку пройде; захист — гард із S8.
- Done when: `test_brief_grounding` зелений по всіх шести підпунктах.
- Verify: `cd server && pnpm exec vitest run test/brief-service.test.ts`
...
- Done when: увесь серверний набір зелений, вендоровані копії ідентичні, лінт і
  типи чисті.
- Verify:
  `./scripts/vendor-shared.sh && ./scripts/vendor-shared.sh --check && bash .claude/skills/pr-self-review/scripts/gates.sh --unit --only server`
...
- Текст — у наявному неймспейсі `prReview` під ключем `brief.*`
  (`client/messages/en/prReview.json`). **`messages/en/brief.json` не чіпати**: його
  ключі описують мертву чотириблокову форму `PrBrief` і є розширювальною точкою.
...
- Файли: `…/_components/OverviewTab/OverviewTab.tsx` (змінено) ·
  `…/_components/OverviewTab/OverviewTab.test.tsx` (нове або доповнено)
...
- Клік по елементу review focus викликає перехід із файлом — проп
  `onOpenFile(path)`, який `PrDetailView` зв'яже у S17 (AC-42).
- Done when: `OverviewTab.test.tsx` доводить AC-36, AC-52, AC-54, AC-55 **із
  мокнутих даних API**, а не з переданих руками пропсів; шелл мокнуто
...
- Done when: `SmartDiffViewer.test.tsx` доводить AC-43 і AC-44; ціль тесту —
  картка, що **не перша** в списку (картка з індексом 0 розгорнута й без
  дип-лінка, `client/INSIGHTS.md` 2026-08-08).
- Verify: `cd client && pnpm exec vitest run src/app/repos/\[repoId\]/pulls/\[number\]/_components/SmartDiffViewer/SmartDiffViewer.test.tsx`
...
**S18 — тексти й доступність**
- Файли: `client/messages/en/prReview.json` (доповнено ключами `brief.*`)
- Skills: `.claude/skills/react-best-practices/SKILL.md` — *Accessibility (HIGH)*
- Плюралі — явні ICU (`{count, plural, one {…} other {…}}`): next-intl без них
  рендерить «1 findings» (`client/INSIGHTS.md`, 2026-08-06).
- Done when: жодного літерала UI-тексту в компонентах; доступні імена двох кнопок
  регенерації на Overview різні.
...
| AC-6 | рівно один структурований виклик п'яти полів | S8, S9 | `test_brief_call` (`server/test/brief-service.test.ts`) | — |
| AC-7 | allowlist із відповіді blast | S10 | `test_brief_grounding` | — |
| AC-8 | `degraded` → allowlist зі шляхів PR | S10 | `test_brief_grounding` | — |
| AC-9 | ризик поза allowlist відкинуто | S10 | `test_brief_grounding` | — |
| AC-10 | focus поза allowlist відкинуто | S10 | `test_brief_grounding` | — |
| AC-11 | частка `N/M` у журналі завжди | S10 | `test_brief_grounding` | спільний тест із NFR-5 |
| AC-12 | `risk_level` зберігається, коли ризики відкинуто | S10 | `test_brief_grounding` | — |
| AC-13 | focus — шлях + причина, без рядка | S8 | `test_brief_call` | — |
...
| AC-22 | вимір токенів до відправлення | S7 | `test_brief_budget` (`server/test/brief-budget.test.ts`) | — |
...
| AC-26 | не влізло після всіх рівнів — гучна відмова | S7 | `test_brief_budget` | стаб `llm()`, що кидає |
...
| AC-35 | кожен недовірений блок позначено | S8 | `test_brief_prompt_guard` (`server/test/brief-prompt.test.ts`) | — |
...
| AC-42 | активація елемента веде на вкладку зі змінами | S16, S17 | `PrBriefCard.test.tsx` | `push`, один `setParams` |
...
| AC-58 | відповідь позначена як незаземлена | S10 | `test_brief_grounding` | — |
| AC-59 | недоступний issue — у переліку недоступних | S4, S11 | `test_brief_input_it` | — |
| AC-60 | гард — останній у системному повідомленні | S8 | `test_brief_prompt_guard` | — |
...
| AC-64 | рівень 4: викликачі понад п'ять на символ | S5, S7 | `test_brief_budget_levels` | per-symbol, не по плоскому списку |
| AC-65 | рівень 5: статистика понад 50 найбільших | S5, S7 | `test_brief_budget_levels` | — |
...
| NFR-1 | ≤8 000 токенів, виміряно лічильником | S6, S7 | `test_brief_budget` | число авторизує вимір S6 |
...
| NFR-4 | нуль незагорнутих недовірених блоків | S8 | `test_brief_prompt_guard` | — |
...
| NFR-8 | нуль тихих скорочень | S7 | `test_brief_budget` | — |
...
| новий шлях у модель | застосований `INJECTION_GUARD` | S8 |
...
3. `bash .claude/skills/pr-self-review/scripts/gates.sh --unit --only client`
4. `cd server && pnpm exec vitest run test/brief.it.test.ts` — DB-залежна половина
   (потрібен Docker; **не** проти стека з живими ранами).
5. Зі **зупиненим** `pnpm dev`: `cd client && pnpm build` — спільний чанк 102 kB
   (NFR-6) і вендорна пастка `.js`→`.ts`.
6. Ручна перевірка на живому стеку (`./scripts/dev.sh`): відкрити PR → картка в
   порожньому стані → «побудувати» → бриф із рівнем ризику, ризиками й Review
   Focus → клік по елементу focus відкриває вкладку зі змінами з розгорнутою
   карткою того файлу → «Назад» повертає на Overview → повторне відкриття PR
   показує чип кешу й **нуль** нових викликів у лозі.
...
- **Інвалідація кешу по `indexed_sha` чи `updated_at` документа** (UX-2): ключ —
  тільки `head_sha`, як записано в спеці.
...
- **Зміна NFR-1 за результатом виміру.** Якщо S6 покаже розходження — число
...
|---|---|---|---|
| Скільки реально важить вхід брифа на PR цього репо | `repo-intel/constants.ts`, `blast/constants.ts`, `context/constants.ts:26`, `intent/constants.ts` — усі капи прочитані; паперова оцінка 5 000–7 000 без документів | Виміряти без піднятої БД і реальних імпортованих PR неможливо, а планування не піднімає стек | S6: `pnpm measure:brief --pr <uuid>` на трьох реальних PR |
| Чи не зробить AC-24 гучну відмову (AC-26) звичайною на великих PR | `server/docs/specs/07-pr-brief.md:295-297` і open question `:600-606`; `intent/constants.ts:MAX_CHANGED_FILES = 60` як контраст | Спека свідомо лишила перелік файлів невідкидним; альтернативу («показано N з M») вона назвала й не взяла | Рішення власника продукту або перший PR у 400 файлів, що впав на AC-26 |
| Чи прийнятно, що в репо з одним документом project context бриф системно будується **без** проєктного контексту | `server/docs/specs/07-pr-brief.md:281-293`; `context/constants.ts:26` | Це прямий наслідок AC-61 + `MAX_DOC_BYTES`; спека це зафіксувала, але не змінила | Рішення власника: або кап на документ у збирачі брифа, або більший бюджет — обидва є зміною вимоги |
| `cl100k_base` — не токенізатор цільової моделі | `adapters/tokenizer/index.ts:31-46`, один лічильник на всі провайдери | Для не-OpenAI провайдерів 8 000 — наближення, і AC-26 вперше робить його вирішальним (гучна відмова) | Порівняння виміру S6 з `usage.prompt_tokens` першої реальної побудови |
| Чи потрібен `ContainerOverrides.blast` для тестів | `platform/container.ts:55-79` — overrides є для `llm`, `github`, `repoIntel`, `tokenizer`, `projectContext`, `contextRepo` | Наявних override-ів вистачає: `BlastService` будується над підміненим `repoIntel`. Додавати запис наперед — вгадувати | Перший тест S10/S11, якому справді не вистачить наявних |

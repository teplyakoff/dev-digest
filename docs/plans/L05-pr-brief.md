...
**Ревізія:** патч після крос-модельного рев'ю плану (`docs/results/l05-homework/plan-cross-model-review.md`).
Змінилися ролі кроків S6, S7 і S8 — див. **примітку про ротацію** на початку
розділу *Steps*; прогін, що піде за старим порядком, збере не той артефакт.
**Assumptions:**
...
читання й після крос-модельного рев'ю плану.

| Знахідка | Вид | Цитата | Власник |
|---|---|---|---|
| **Ризик без жодного посилання проходить заземлення порожньо.** AC-9 відкидає ризик, що посилається **поза** allowlist; ризик із `file_refs: []` не посилається нікуди, тож формально виживає — а *Goals* спеки вимагають «заземлити кожен ризик». Вимога «щонайменше одне посилання на ризик» — новий критерій, не інтерпретація | consistent (Goals ↔ AC-9) | AC-9 `server/docs/specs/07-pr-brief.md:184-185`; Goals `:62-64`; `Risk.file_refs` без `.min(1)` — `server/src/vendor/shared/contracts/brief.ts:69-75` | `spec-creator` — план реалізує AC-9 **як записано**, а розрив тримає видимим (інструкція в промпті S6 + лічильник у лозі S10) |
...
| Ключ кешу — лише `head_sha`, тож зміна моделі або тексту промпту не інвалідує бриф на тому ж комміті | bounded | `server/docs/specs/07-pr-brief.md` open question «ключ кешу — лише `head_sha`»; AC-16 `:213-215` | `spec-creator` — план **не** додає інвалідації сам; рядок у *Open decisions* |
...
- У промпті є **інструкція**: кожен ризик має назвати щонайменше одне посилання на
  файл або endpoint із входу. Це не AC — див. *Requirements review* і *Open
  decisions*; це найдешевше, що можна зробити, не вигадуючи вимогу.
...
- **Без `.max()` і без `.min()` у самій схемі** — стеля застосовується після парсу
  зрізом (`intent/pipeline/schema.ts:21-31` фіксує, у що обійшлася стеля в схемі:
  модель, що на одиницю перевищила ліміт, після ре-промпту стала різко біднішою
  при подвоєних вихідних токенах).
...
- Ризик або елемент focus поза своїм списком **відкидається** (AC-9, AC-10) — це
  дзеркало `groundFindings` (`reviewer-core/src/grounding.ts:52-83`): перевіряється
  належність до списку, а не збіг тексту.
- Рядок логу з часткою `N/M` пишеться **безумовно** — і коли відкинуто, і коли ні
  (AC-11, NFR-5; правило з `server/INSIGHTS.md`, 2026-08-06). У тому ж рядку —
  **скільки ризиків прийшло без жодного посилання**: AC-9 їх не відкидає (див.
  *Requirements review*), тож єдине, що план може зробити, не вигадуючи вимоги, —
  не дати цьому бути невидимим.
...
- Done when: `test_brief_grounding` зелений по всіх підпунктах, включно з
  «`review_focus.path`, що дорівнює маршруту endpoint, відкинуто».
- Verify: `cd server && pnpm exec vitest run test/brief-service.test.ts`
...
| AC-9 | ризик поза allowlist відкинуто | S10 | `test_brief_grounding` | ризик без посилань — див. *Open decisions* |
...
- **Відкидання ризику без жодного посилання** — це новий критерій, не інтерпретація
  AC-9; план його не вводить, а лише робить видимим у лозі (S10).
...
| Чи має ризик без жодного посилання виживати заземлення | AC-9 `…:184-185`, Goals `…:62-64`, `Risk.file_refs` без `.min(1)` (`brief.ts:69-75`) | «Щонайменше одне посилання» — нова поведінка, тобто новий критерій; план не вигадує вимог. Сьогодні тримається інструкцією промпту (S6) і лічильником у лозі (S10) | Рішення `spec-creator`: новий AC або явне «так і має бути» |
| Чи законно перевіряти `review_focus.path` проти **підмножини** allowlist (без маршрутів endpoints) | AC-7 `…:177-179`, AC-10 `…:187-188`, клієнтський AC-42/AC-44 | AC-10 вимагає відкидати те, що поза списком, і не забороняє відкидати більше; підмножина строгіша й дає клієнту тільки те, що він може відкрити. Але це **інтерпретація**, і вона має бути видимою | Підтвердження власника або окремий AC про різні списки для ризиків і focus |
| Чи прийнятно, що зміна моделі або тексту промпту не інвалідує бриф на тому ж `head_sha` | AC-16 `…:213-215`, open question спеки про ключ кешу | Ключ кешу — вимога; додати до нього версію промпту означало б змінити AC-16 | Рішення `spec-creator` (наприклад, `prompt_version` у ключі) |
| Чи не зробить AC-24 гучну відмову (AC-26) звичайною на великих PR | `…:295-297` і open question `:600-606`; `intent/constants.ts:MAX_CHANGED_FILES = 60` як контраст | Спека свідомо лишила перелік файлів невідкидним; альтернативу («показано N з M») вона назвала й не взяла | Рішення власника продукту або перший PR у 400 файлів, що впав на AC-26 |
| Чи прийнятно, що в репо з одним документом project context бриф системно будується **без** проєктного контексту | `…:281-293`; `context/constants.ts:26` | Прямий наслідок AC-61 + `MAX_DOC_BYTES`; спека це зафіксувала, але не змінила | Рішення власника: кап на документ у збирачі брифа або більший бюджет — обидва є зміною вимоги |

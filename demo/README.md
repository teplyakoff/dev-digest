...
| `npm run record:brief` | the L05 homework PR Why + Risk Brief: an Overview with no brief → one structured call fills the card with the risk level, why/what and the risks → Review focus, every path checked against the blast map → a focus item followed into the changes tab, that file's card open → the same state re-opened, served from the stored row for **zero** model calls → Rebuild forcing a fresh build | **real money** (two builds) |
| `npm run record:context` | the L05 lab Project Context: the sidebar entry → every `.md` in the repo offered for import → a document's text beside the list → the per-row counter going none → one → two as it is attached, live → two more onto a **skill**, whose rows then count the agents that link it → a real run reporting `Project context: 3/3 document(s) loaded` and Prompt assembly naming all three | **real money** (one review) |
| `npm run record:evals` | the L06 eval pipeline: a decided finding → **one click** turns it into an eval case, with no modal and an "Edit case" link in the toast → the agent's Evals tab, MUST FIND / MUST NOT FLAG badges and "no model call in the scorer" → Run all evals, the action disabled, the metrics landing → one instruction removed from the system prompt and a second batch → the Eval Dashboard from the sidebar → two runs selected, Compare: the four deltas and the prompt diff | **real money** (one batch per set of cases, ×2 on the prompt-vs-prompt path) |

...
description: "Maintains the per-package INSIGHTS.md knowledge files. Use at the START of any work inside server/, client/, reviewer-core/ or e2e/ to read that package's INSIGHTS.md and name the top 3 entries relevant to the task; use at the END of a session, or the moment something non-obvious happens, to extract what was learned and append it to the INSIGHTS.md of the package the task actually touched. Trigger terms: insights, learnings, wrap-up, retrospective, session summary, capture what we learned, end of session, INSIGHTS.md, LEARNINGS.md."
---

# engineering-insights

Every package keeps its own `INSIGHTS.md` — knowledge lives next to the code it
is about. This skill has two modes; read
